...
async function getCountryBorderByName(name: string) {
  ...
  const countries: Country[] = await resp.json();
...
const CountryDetail = async ({ params }: ICountryDetail) => {
  const country = await getCountryByName(params.name);
  const formatter = Intl.NumberFormat("rus", { notation: "compact" });
  const borderCountries = await getCountryBorderByName(decodeURI(params.name));
